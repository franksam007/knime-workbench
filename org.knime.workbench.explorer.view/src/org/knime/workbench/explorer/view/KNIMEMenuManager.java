 ...
 * opened on a selection of the workflow hub.
 *
...
public class KNIMEMenuManager extends MenuManager {
    ...
    /* TODO: check if instanceof ExplorerAction is enough, third party actions could derive from these and still add
     * their actions, easier would be to set another boolean after we added our internal stuff, however, then there's no
     * possibility at all to add third party actions, do we want to really enforce that? */
     ...
     *
     * TODO: define external actions, currently: not an instanceof ExplorerAction
     *
     * @param allow <code>true</code> if external actions shall be allowed, <code>false</code> otherwise.
     */
    public void setAllowExternalActions(final boolean allow) {
    ...
    public void add(final IContributionItem item) {
        if (m_allowExternalActions || item instanceof ExplorerAction) {
            super.add(item);
        }
    }
