...
public abstract class MetaInfoAtom {
    ...
    /**
     * Consumers who want to know when a metadata info atom has been deleted.
     */
     ...
     */
    protected static RenderSizeData calculateRenderSize(final Drawable widget, final String text,
        final int horizontalInset, final int verticalInset, final boolean renderNAry) {
        final GC gc = new GC(widget);

        gc.setFont(WorkflowMetaView.NOT_AUTHOR_FONT);
        final Point tagTextSize = gc.textExtent(text);

        final Point nArySize;
        if (renderNAry) {
            gc.setFont(WorkflowMetaView.CLOSE_N_ARY_FONT);
            nArySize = gc.textExtent(N_ARY_TIMES);
            nArySize.y = nArySize.x;    // it's an equi-sided X, but fonts usually give it a bottom inset
        } else {
    ...
    private final CopyOnWriteArrayList<DeletionListener> m_listeners;
     ...
     */
    public void addChangeListener(final DeletionListener listener) {
        m_listeners.add(listener);
    }

    /**
     * Consumers which have added themselves via {@link #addChangeListener(DeletionListener)} can remove themselves via
     * this method.
     *
     * @param listener an implementor
     */
    public void removeChangeListener(final DeletionListener listener) {
        m_listeners.remove(listener);
    }

    /**
     * Subclasses should invoke this method when an applicable add or delete action has transpired.
     */
    protected final void messageListeners() {
    ...
    protected void save(final TransformerHandler parentElement, final String form)
        ...
        parentElement.startElement(null, null, MetadataXML.ELEMENT, attributes);
        final char[] value = m_value.toCharArray();
        parentElement.characters(value, 0, value.length);
        parentElement.endElement(null, null, MetadataXML.ELEMENT);
    }
     ...
     */
    protected static class RenderSizeData {
    ...
    protected abstract class CloseableLabel extends Canvas {
        ...
        protected CloseableLabel(final Composite parent, final boolean forEdit, final int horizontalInset,
            ...
            setFont(WorkflowMetaView.NOT_AUTHOR_FONT);
